...
var LEVELUP_RETR_CONCURRENCY = 10;
...
function Datastore (options) {
  ...
  // LevelUP
  this.store = stores[options.filename] = stores[options.filename] || levelup(options.filename, options.store || { });
...
Datastore.prototype.buildIndexes = function (cb) {
  ...
  if (_.every(self.indexes, function(idx) { return idx.ready })) return cb(null);

  // For now only full rebuild, but must be index specific
  self.resetIndexes();

  self.store.createReadStream()
  .on("data", function (data) {
    self.addToIndexes(JSON.parse(data.value));
  })
  .on("end", function() {
    _.each(self.indexes, function(idx) { idx.ready = true });
    cb(null);
  });
};
...
Datastore.prototype.getCandidates = function (query, sort, callback) {
  ...
  // Temporary use process.nextTick, otherwise we will wait for index building ; TODO
  self.buildIndexes(function() {
...
Datastore.prototype.insert = function (newDoc, cb) {
  ...
  async.each(util.isArray(newDoc) ? newDoc : [newDoc], function(d, cb) { 
    self.store.put(d._id, model.serialize(d), cb);
  }, function(err) {
  ...
  this.getCandidates(query, null, function(candidates) {
    try {
      candidates.forEach(function (d) {
        if (model.match(d, query) && (multi || removed.length === 0)) {
          removed.push(d._id);
