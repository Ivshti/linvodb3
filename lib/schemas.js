...
function normalize(schema) {
	_.each(schema, function(spec, key) {
		if (isComplexSpec(spec)) return;
		else if (util.isArray(spec)) schema[key] = { schema: typeof(spec[0])=="string" ? spec[0] : undefined, type: "array" };
...
function construct(self, schema) {
	...
	_.each(schema, function(spec, key) {
		if (spec.get || spec.set) return Object.defineProperty(self, key, spec); // spec is of getter/setter type
