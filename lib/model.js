...
Model.prototype.insert = function (newDoc, cb) {
  ...
  self._pipe.push(function(callback) {
    ...
    async.map(util.isArray(newDoc) ? newDoc : [newDoc], function(d, cb) { 
      d = (new self(d));
      self.emit("insert", d);
      d._persist(cb);
    }, function(err, docs) {
...
Model.prototype.save = function (newDoc, cb) {
  // TODO: this should be modified to support arrays (bulk save)
  // We'll run through all the docs, give them an ID if we don't have one
  // Then, we save them all via _persist (see Model.insert) and finally we update all the indexes 
  // We do that by going through all docs, and call .updateIndexes or .addToIndexes depending on whether it's old/new - and for that we might have to query the DB

  // More specifically build an array "existingDocs" when making the query, then we have to index by _id and pair them with the given array to pass to updateIndexes
