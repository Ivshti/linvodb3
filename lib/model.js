...
Model.prototype.update = function (query, updateQuery, options, cb) {
  ...
  stream.on("error", function(e) { err = e; stream.close() });
  stream.on("ids", function(ids) {
    ...
    stream.on("data", function(data) {
      try {
        if (! document.match(data.val(), query)) return; // Not a match, ignore
      } catch(e) { err = e; stream.close(); return; }

      try {
        var val = data.lock(); // we're doing a modification, grab the lock - means this _id will be served from memory (through the ref until we unlock it
        data.newDoc = document.modify(val, updateQuery); // while the object is locked, the same reference is returned for all .lock() calls
        data.oldDoc = document.deepCopy(val);
        _.extend(val, data.newDoc); // don't update on .modify, in case we emit an error while modifying
    ...
    stream.on("ready", function() {
      ...
      async.each(modifications, function(d, cb) {
        self.store.put(d.id, document.serialize(d.newDoc), function(err) {
          d.unlock();
          cb(err);
        });
      }, function(err) {
        callback(err || null, err ? undefined : modifications.length);
      });
    });
  });
};
...
Model.prototype.getCandidates = function (query, sort, cb) {
  var stream = Cursor.getMatchesStream(this, query, sort), res = [];
  stream.on("ids", function(ids){ res = new Array(ids.length) });
  stream.on("data", function(d) { res[d.idx] = d.val() });
  stream.on("ready", function() { cb(res) });
};
...
Model.prototype.remove = function (query, options, cb) {
  ...
  var stream = Cursor.getMatchesStream(this, query);
  stream.on("data", function(d) {
    try {
      var v = d.val();
      if (document.match(v, query) && (multi || removed.length === 0)) {
        removed.push(v._id);
