 ...
 */
Model.prototype.save = function (docs, cb) {
  var self = this;
  ...
  stream.on("ready", function() {
    ...
    async.each(modifications, function(m, cb) {
      if (! m.oldDoc) self.emit("insert", m.newDoc);
      m.newDoc._persist(cb);
    }, function(err) {
      ...
      if (inserted.length) self.emit("inserted", inserted);
      if (updated.length) self.emit("updated", updated);
