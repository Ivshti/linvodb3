...
Model.prototype.update = function (query, updateQuery, options, cb) {
  ...
  stream.on("ids", function(ids) {
    ...
    stream.on("data", function(data) {
      ...
      try {
        data.lock(); // we're doing a modification, grab the lock - means this _id will be served from memory (through .val()) until we unlock it
        data.newDoc = document.modify(data.val(), updateQuery); // while it's locked, the reference to .val() cannot change
        data.oldDoc = document.deepCopy(data.val());
