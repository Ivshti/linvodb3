...
function Model (name, schema, options) {
  ...
  var self = function Document(raw) { Instance.call(this, self, raw) }; // Call the instance builder
...
Model.prototype.update = function (query, updateQuery, options, cb) {
  ...
  stream.on("ids", function(ids) {
    ...
    stream.on("ready", function() {
      ...
      // Persist in LevelUP
      async.each(modifications, function(d, cb) {
        self.store.put(d.id, document.serialize(d.newDoc), function(err) { d.unlock(); cb(err) });
      }, function(err) {
        callback(err || null, 
          err ? undefined : modifications.length,
          (!err && modifications.length) ? modifications[0].newDoc : undefined
        );
      });
    });
  });
