...
function Model (name, schema, options) {
  ...
  var self = function Document(raw) { return Instance.call(this, self, raw) }; // Call the instance builder
  ...
  self.schema = schema;
  self.filename = path.normalize(options.filename || name+".db");
  ...
  schemas.getIndexes(schema).forEach(function(idx) { self.ensureIndex(idx) });

  // Concurrency control for 1) index building and 2) pulling objects from LevelUP
  self._pipe = new bagpipe(1);
  self._retrQueue = async.queue(Cursor.retriever, LEVELUP_RETR_CONCURRENCY);
 ...
 */
function Instance(db, raw) 
{
  var self = this, raw = raw || { };

  // We should handle this in schemas as default properties; we have to set the values here because if raw is Document they won't be enumerated by _.extend
  Object.defineProperty(this, "_ctime", { enumerable: false, writable: true, value: raw._ctime }); 
  Object.defineProperty(this, "_mtime", { enumerable: false, writable: true, value: raw._mtime }); 

  Object.defineProperty(this, "save", { value: function(cb) {
    return db.save(self, cb);
  } });

  Object.defineProperty(this, "remove", { value: function(cb) {
    if (! self._id) return cb(); // TODO maybe emit error
    return db.remove({ _id: self._id }, cb);
  } });

  Object.defineProperty(this, "update", { value: function(modifier, cb) {
    if (self._id === undefined) self._id = db.createNewId();
    return db.update({ _id: self._id }, modifier, { upsert: true }, cb);
  } });

  // Persist to LevelUP
  Object.defineProperty(this, "_persist", { value: function(cb) {
    self._mtime = new Date();
    db.store.put(self._id, document.serialize(self), function(err) {
...
Model.prototype.initStore = function() {
  var filename = this.filename = path.normalize(this.options.filename || name+".db");
...
Model.prototype.insert = function (newDoc, cb) {
  ...
  self._pipe.push(function(callback) {
    ...
    async.map(util.isArray(newDoc) ? newDoc : [newDoc], function(d, cb) { (new self(d))._persist(cb) }, function(err, docs) {
  ...
  stream.on("ids", function(ids) {
    ...
    stream.on("data", function(data) {
      ...
      try {
        ...
        data.oldDoc = document.deepCopy(val);
    ...
    stream.on("ready", function() {
      ...
      async.map(modifications, function(d, cb) {
        (new self(d.newDoc))._persist(function(err, doc) { d.unlock(); cb(err, doc) });
      }, function(err, docs) {
        callback(err || null, 
          err ? undefined : docs.length,
          (!err && docs.length) ? docs[0] : undefined
        );
      });
    });
  });
 ...
 */
Model.prototype.save = function (newDoc, cb) {
  if (newDoc._id === undefined) newDoc._id = this.createNewId();
  return this.update({ _id: newDoc._id }, newDoc, { upsert: true }, function(err, c, doc) { cb(err, doc) });
};
