...
function Model (name, schema, options) {
  ...
  getIndexes(schema).forEach(function(index) { self.ensureIndex(index) });
  // TODO: schemas.getIndexes(schema).forEach(self.ensureIndex.bind(self));
...
/* 
 * Instance of a model
 * Low level functions here (save/remove), we use them in our higher-levels (Model.update, Model.remove)
 */
function Instance(db, raw) 
{
  ...
  Object.defineProperty(this, "remove", { value: function(cb) {
    if (! self._id) return; // TODO maybe emit error
    return db.remove({ _id: self._id }, cb);
  } });

  Object.defineProperty(this, "update", { value: function() {
    // TODO
  } });

  // Persist to LevelUP
  Object.defineProperty(this, "_persist", { value: function(cb) {
    if (self._id === undefined) self._id = db.createNewId(); // set ctime
    // set mtime
