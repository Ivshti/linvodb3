...
Cursor.getIdsForQuery = function(db, query, sort) {
  ...
  _.each(query, function(value, key) {
    ...
    if (index && (indexed || !db.options.autoIndexing)) {
      ...
      else if (value.hasOwnProperty("$exists") && value.$exists==true) push(index.getAll(true)); // .getAll(sparse) - returns the sparse part of the idx
      else if (value.hasOwnProperty("$exists") && value.$exists==false) exclude(index.getAll(true)); 
    };
  });
