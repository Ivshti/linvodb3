...
Cursor.prototype.exec = function(callback) {
  ...
  var limit = function(res, limit, skip) { return res.slice(skip, limit + skip) };
  ...
  stream.on("ids", function(ids) {
    var doLimit = ids._indexed && ids._sorted && !self._filter;
    if (doLimit) ids = limit(ids, self._limit || ids.length, self._skip || 0);
    stream.trigger(ids); // Start retrieving those IDs

    res = new Array(ids.length);
    stream.on("data", function(d) { res[d.idx] = d.val() });
    stream.on("ready", function() {
      try {
        if (!ids._indexed) res = res.filter(function(x) { return document.match(x, self.query) });
        if (!ids._sorted && self._sort) res = res.sort(Cursor.getSorter(self._sort));
