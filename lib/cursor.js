...
Cursor.getIdsForQuery = function(db, query, sort) {
  ...
  _.each(query, function(value, key) {
    ...
    if (index && (indexed || !db.options.autoIndexing)) {
      ...
      else if (value.hasOwnProperty("$ne")) excludes = _.union(excludes, index.getMatching(value.$ne));
      else if (value.hasOwnProperty("$in")) push(index.getMatching(value.$in));
      else if (value.hasOwnProperty("$nin")) excludes = _.union(excludes, index.getMatching(value.$nin));
