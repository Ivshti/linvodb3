...
Cursor.prototype.exec = function(callback) {
  ...
  var stream = Cursor.getMatchesStream(this.db, this.query, sort);
  ...
  stream.on("ids", function(ids) {
    ...
    stream.on("data", function(d) { 
      ...
      if (self._live) resIds[v._id] = true; // Keep those in a map if we need them for a live query
...
Cursor.prototype.live = function(query) {
  ...
  // Watch for changes
  this.db.on("updated inserted", function(docs) {
    // TODO: refresh if any of the objects: have an ID which is in our results OR they match our query (this.query)
  });
  this.db.on("removed", function(ids) {
    // TODO: refresh if any of the objects: have an ID which is in our results
  }); 
