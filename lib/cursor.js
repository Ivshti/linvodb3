...
Cursor.getIdsForQuery = function(db, query, sort) {
  ...
  var res = null, excludes = []; // TODO get this from sort if possibru
  ...
  _.each(query, function(key, value) {
    ...
    if (! db.indexes[key]) {
      indexed = false;
      if (AUTO_INDEXING) db.ensureIndex({ fieldName: key });
    };
    
    if (db.indexes[key] && (indexed || !AUTO_INDEXING)) { // 1) We can utilize this index and 2) 
...
//  if (! indexed) return null; // TODO: don't return if auto-indexing is disabled; we also need to keep building results
...
*/
  ...
  if (usableQueryKeys.length > 0) {
    return self.indexes[usableQueryKeys[0]].getMatching(query[usableQueryKeys[0]]);
  }
  ...
  usableQueryKeys = _.intersection(usableQueryKeys, indexNames);
  if (usableQueryKeys.length > 0) {
    return self.indexes[usableQueryKeys[0]].getMatching(query[usableQueryKeys[0]].$in);
  }
  ...
  usableQueryKeys = _.intersection(usableQueryKeys, indexNames);
  if (usableQueryKeys.length > 0) {
    return self.indexes[usableQueryKeys[0]].getBetweenBounds(query[usableQueryKeys[0]]);
  }
