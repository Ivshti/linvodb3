...
Cursor.getIdsForQuery = function(db, query, sort) {
  ...
  if (sort) _.each(sort, function(value, key) {
    push(Cursor.getIdsForQuery(db, _.pick(query, key)));
  ...
  _.each(query, function(value, key) {
    // Don't match on any of the sort keys, since we've done this already in the sort loop
    ...
    if (index && (indexed || !db.options.autoIndexing)) {
      ...
      if (value.hasOwnProperty("$ne")) exclude(index.getMatching(value.$ne));
      if (value.hasOwnProperty("$in")) push(index.getMatching(value.$in));
      if (value.hasOwnProperty("$nin")) exclude(index.getMatching(value.$nin));
      if (value.hasOwnProperty("$lt") || value.hasOwnProperty("$lte") || value.hasOwnProperty("$gt") || value.hasOwnProperty("$gte")) push(index.getBetweenBounds(value));
      if (value.hasOwnProperty("$exists") && value.$exists==true) push(index.getAll(function(n) { return n.key!==undefined }));
      if (value.hasOwnProperty("$exists") && value.$exists==false) push(index.getAll(function(n) { return n.key===undefined })); 
      if (value.hasOwnProperty("$regex")) { var r = new RegExp(value.$regex); push(index.getAll(function(n) { return n.key && n.key.match(r) })); }
    };
  });
