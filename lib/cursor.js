...
Cursor.prototype.exec = function(callback) {
  ...
  this.db.emit("query", this.query);

  var stream = Cursor.getMatchesStream(this.db, this.query, sort, this._ids);
  ...
  stream.on("ids", function(ids) {
    ...
    var earlyMapReduce = earlyLimit && earlySort && self._map && self._reduce; // special mode - run the map/reduce directly on data event
    ...
    if (earlyMapReduce) res = self._reduce[1] || undefined;
    ...
    stream.on("data", function(d) { 
      ...
      if (self._live) resIds[v._id] = v; // Keep those in a map if we need them for a live query
      ...
      if (earlyMapReduce) { // The early map-reduce system, map/reduce-es results on the go if we can (results are pre-sorted and limited)
        try {
          v = self._map(v);
          res = res===undefined ? v : self._reduce[0](res, v);
        } catch (e) { stream.close(); err = e; res = undefined };
        return;
      }
    ...
    stream.on("ready", function() {
      ...
      try {
        ...
        if (self._reduce) res = res.reduce.apply(res, self._reduce);
...
Cursor.prototype.live = function(query) {
  ...
  if (this._live) return this; // Live query already initialized
  ...
  // Refresh live query
  var refresh = debounce(function(callback) {
    self.exec(function(err, res) {
      if (err) console.error(err); // No other way for now
      self.res = res;
      self.db.emit("liveQueryUpdate", query);
      callback();
    });
  }, LIVE_QUERY_DEBOUNCE);
  refresh();
  ...
  function updated(docs) {
    ...
    if (docs.some(function(doc) {
      var exists = self._ids[doc._id];
      delete self._ids[doc._id]; // Delete that so we re-fetch it from the DB
      return exists || document.match(doc, self.query);
    })) refresh();
  };
  ...
  this.db.on("reload", function() { refresh() }); // Refresh on DB reload
...
Cursor.getMatchesStream = function(db, query, sort, prefetched) {
  ...
  stream.on("ids", stream.trigger = function(ids) {
    ...
    ids.forEach(function(id, i) {
      db._retrQueue.push({ stream: stream, id: id, idx: i, db: db, prefetched: prefetched && prefetched[id] }, function() {
...
Cursor.retriever = function(task, cb) {
  ...
  if (task.prefetched) { task.stream.emit("data", { 
...
Cursor.getIdsForQuery = function(db, query, sort) {
  ...
  function match(key, val) {
    ...
    if (index && (indexed || !db.options.autoIndexing)) {
      ...
      if (val && val.hasOwnProperty("$regex")) { var r = new RegExp(val.$regex); push(index.getAll(function(n) { return n.key && n.key.match(r) })); }
    };
  };
