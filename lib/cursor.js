...
Cursor.prototype.exec = function(callback) {
  ...
  var reducer;
  if (self._reduce) reducer = [self._reduce[0], typeof(self._reduce[1]) === "object" ? document.deepCopy(self._reduce[1]) : self._reduce[1]]; // we have to copy the initial value, so that we don't inherit old vals
