...
/*
  ...
  stream.on("ids", function() {
    // TODO If a query is fully sorted OR there is no .sort supplied 
    //  (fully sorted flag should be set to true if there is no .sort supplied), apply skip and limit here via .slice()
    // Do tests for ._sorted and ._indexed flags

    // Scanner
    // TODO If a query is fully indexed, don't call document.match; call document.match with a .filter otherwise

  });
*/

  this.db.getCandidates(this.query, this._sort, function(candidates) {
    try {
...
Cursor.getIdsForQuery = function(db, query, sort) {
  ...
  sorted = firstKey ? (!!db.indexes[firstKey] && _.keys(sort).length==1) : true; // TODO; change the logic here when we have compound indexes
