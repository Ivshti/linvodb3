 ...
 */
Cursor.prototype.reduce = function(reduce) {
  if (typeof(reduce)=="function") this._reduce = reduce;
  return this;
};
...
Cursor.prototype.exec = function(callback, countOnly) {
  ...
  stream.on("ids", function(ids) {
    ...
    var doLimit = indexed && sorted && !self._filter && !sorter; // query is indexed, sorter and we don't have special filter/sort funcs
    ...
    stream.on("ready", function() {
      try {
        ...
        if ((!sorted && sort) || sorter) res = res.sort(sorter || Cursor.getSorter(sort));
        if (!doLimit) res = limit(res, self._limit || res.length, self._skip || 0);

        // TODO: if we don't have sort/limit/skip we can do map/reduce while running the query, in data event
        if (self._map) res = res.map(self._map);
        if (self._reduce) res = res.reduce(self._reduce);
...
Cursor.getIdsForQuery = function(db, query, sort) {
  ...
  function match(key, val) {
    ...
    if (key[0] == "$") {
      if ((key=="$and" || key=="$or") && util.isArray(val)) {
